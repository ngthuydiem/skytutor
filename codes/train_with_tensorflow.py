...
flags.DEFINE_integer("embedding_size", 256, "The embedding dimension size.") #200
flags.DEFINE_integer(
	"epochs_to_train", 5,
	"Number of epochs to train. Each epoch processes the training data once completely.") # 15
flags.DEFINE_float("learning_rate", 0.025, "Initial learning rate.")
flags.DEFINE_integer("num_neg_samples", 5,
					 "Negative samples per training example.") # 25
...
#"model.analogy('france', 'paris', 'russia') and "
...
class Word2Vec(object):
  ...
  def train(self):
	...
	while True:
	  ...
	  (epoch, step, words,lr) = self._session.run([self._epoch, self.step, self._words, self._lr])
	  now = time.time()
...
def main(_):
  ...
  with tf.Graph().as_default(), tf.Session() as session:
	...
	model.saver.save(session, os.path.join(opts.save_path, "model.ckpt"), global_step=model.step)	
